...
<p>Ens posem en contacte amb vosaltres en referència als serveis d'allotjament (hosting) que actualment teniu contractats amb ILIMIT. Com us vem comentar en un correu previ, és necessari actualitzar la versió de PHP de la plataforma d'hostatjament per donar un millor servei, oferir les noves característiques de les versions actuals i disposar d'un entorn amb les darreres actualitzacions en matèria de seguretat per evitar quedar exposat a riscos innecessaris.</p>
</br>
<p>El (<?= $date ?>) començarem el procés de migració del vostre web i correu cap a una nova plataforma de hosting amb versions al dia i actualitzables amb la qual poder brindar-vos el millor suport.  Des de ILIMIT som conscients que aquests moviments us poden crear alguns inconvenients, però esperem que entengueu que són causa natural de l'evolució de la tecnologia i de cara a una millora en la seguretat de tots.</p>
</br>
<p>Hem revisat i vist que disposeu dels següents dominis com a Àlies de correu:</p>
</br>
- <?= $host ?> (principal).
- Domini2 (àlies).
- Domini3 (àlies).

Amb el nou sistema hi haurà un domini principal i els restants serán redirigits. Això significa que a partir d'ara, els correus s'enviaran sempre des del domini principal (l'autenticació contra el servidor s'haurà de fer amb aquest domini) i dels altres seguireu rebent correus. Si ho preciseu, també podreu canviar el From al vostre client de correu per fer veure que envieu des d'algún dels dominis redirigits (en compte de des del principal).
